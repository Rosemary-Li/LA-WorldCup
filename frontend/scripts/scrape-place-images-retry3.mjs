// Retry pass 3: cover the 10 hotels still missing images.
// For each hotel, hand-pick a Wikipedia article that yields a representative photo:
//   - Brand chain article (Marriott, W Hotels, Andaz, etc.) for chain properties.
//   - Specific Wikipedia article when the property has one (Hotel Figueroa).
//   - Neighborhood article when nothing else fits (Westwood, Sunset Strip).
...
const OVERRIDES = {
  // Boutique hotels with no Wikipedia article → neighborhood / category photo
  "petit ermitage":                   "Sunset Strip",
  "plaza la reina":                   "Westwood, Los Angeles",
  "royal palace westwood hotel":      "Westwood, Los Angeles",

  // Chain properties → brand article (Wikipedia infobox usually shows a flagship)
  "andaz west hollywood":             "Andaz",
  "best western plus sunset plaza":   "Best Western",
  "ramada plaza by wyndham":          "Ramada",
  "w los angeles - west beverly":     "W Hotels",
  "courtyard l.a. live":              "Courtyard by Marriott",
  "moxy downtown los angeles":        "Moxy Hotels",

  // Has its own article
  "hotel figueroa":                   "Hotel Figueroa",
};
...
async function main() {
  ...
  // Group by article so duplicate articles only fetch once
  ...
  console.log(`Hotel retry: ${items.length} unique articles covering ${Object.keys(OVERRIDES).length} hotels...`);
