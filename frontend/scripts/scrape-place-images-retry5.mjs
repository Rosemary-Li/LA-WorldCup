// Retry pass 5: restaurants only. Force JPG output.
// Replaces 9 non-jpg restaurant images (likely logos w/ transparent bg) and fills 1 missing.
// Strategy: pick a Wikipedia article whose infobox image is a JPG photo; reject anything
// non-jpg returned by the API. The originals remain on disk as orphans (harmless).
...
const OVERRIDES = {
  "norah":                     "American cuisine",
  "zinqué":                    "French cuisine",
  "zinque":                    "French cuisine",
  "the boiling crab":          "Seafood boil",
  "in-n-out burger":           "Hamburger",
  "eggslut":                   "Egg sandwich",
  "holbox":                    "Yucatecan cuisine",
  "hae jang chon":             "Korean barbecue",
  "pasjoli":                   "Bistro",
  "cabra la":                  "Pollo a la brasa",   // was missing
};
...
// Stricter: only accept JPG. Wikipedia infobox photos are nearly always JPG.
...
async function processWiki(title, slug) {
  ...
  if (!isJpg(ct, imgUrl)) throw new Error(`not a jpg (ct=${ct || "?"}, url ext=${imgUrl.match(/\.([a-z0-9]{2,5})(?:$|\?)/i)?.[1] || "?"})`);
...
async function main() {
  ...
  // Group by article (zinqué + zinque share, may share more in future)
  ...
  console.log(`Restaurant retry: ${items.length} unique articles for ${Object.keys(OVERRIDES).length} restaurants...`);
