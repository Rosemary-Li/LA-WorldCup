// ═══════════════════════════════════════════════════
//  SECTION: NAV
// ═══════════════════════════════════════════════════

document.getElementById('mount-nav').innerHTML = `
    <nav>
      <div class="nav-masthead">
        <div class="nav-title">
          A cinematic journey through football, glamour &amp; the City of Angels — where the silver screen meets the beautiful game.
        </div>
        <div class="nav-logo">LA × WC26</div>
      </div>
      <div class="nav-bar">
        <div class="nav-rule">Est. June 11, 2026</div>
        <ul class="nav-links">
          <li><a href="#matches">Matches</a></li>
          <li><a href="#la-showcase">Explore LA</a></li>
          <li><a href="#itinerary">Journey</a></li>
          <li><a href="#explore">Map</a></li>
          <li><a href="#discover">Hotels & Dining</a></li>
          <li><a href="#about">About Us</a></li>
        </ul>
        <div class="nav-rule">SoFi Stadium · Inglewood</div>
      </div>
    </nav>
`;
