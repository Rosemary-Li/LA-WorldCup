...
原始 Excel / CSV
    ...
    → 个性化排序层       frontend/src/lib/personaSort.js   （Explore 卡片排序）
...
5. **Explore LA** — 杂志风排版入口（HOTEL · Restaurant · Show · Attraction · 竖排 FAN EVENT）+ 悬停切换的特色照片。点任一类别进入卡片+地图分屏视图，并附带杂志风**搜索栏**（边输入边过滤 name / area / region / type / flavor）；卡片会按**用户画像**自动重排 — 一旦 Journey 表单填了任何偏好，列表就按"预算契合度 + 到锚点距离（餐厅锚到已选酒店；否则锚到 SoFi）+ vibe 命中"重新排序。卡片上方的 `✦ Sorted for you · …` 横幅说明当前生效的信号，并提供一键切回默认顺序的按钮。"Build My Journey →" 触发行程生成。
...
│       │   └── personaSort.js    # Explore 卡片个性化排序（预算 · 距离 · vibe）
│       ├── sections/
│       │   ├── PhotoHero.jsx
│       │   ├── Matches.jsx       # 多选赛程
...
Explore LA 的卡片会根据 Journey 表单的偏好实时重排，让"同一份数据库"对每个用户都不一样。完全跑在前端、确定性输出、可一键切回默认顺序。

**信号链路**（全程内存态，不打 API）：

```text
Journey 表单 { type, budget, vibe }      Explore 已选 picks
        │                                       │
        └────────────► main.jsx state ◄─────────┘
                            │
...
| 类别 | 项 1 | 项 2 |
|---|---|---|
| Hotels | `|价格档 − 预算目标档| × 10` | 到 SoFi 的距离 km × 0.5 |
| Restaurants | `|价格档 − 预算目标档| × 5` | 到已选酒店的距离 km × 0.5 — 没选酒店时回落到 SoFi |
| Events / Shows / Attractions | 若 `item.type` ∈ `VIBE_TO_TYPES[vibe]` 则 `−10` | 距离 × 0.5 |

**距离锚点：** 用户在 Explore 已选的第一家酒店；尚未选时回落到 SoFi Stadium（`33.953, −118.339`）。

**价格档：** 酒店 `100+`/`200+`/`400+`/晚 → 1/2/3；餐厅 `$10-20` … `$100+` → 1…5；预算目标档 `Essentials → 1`（酒店）/`2`（餐厅），`Comfort → 2/3`，`Premium → 3/4`。

**Vibe → type 标签：** `football → Watch Party · Official · MLS · Sports · Bar/Party · Meetup`，`culture → Landmark · Museum · Community · Special`，`beach → Beach & Coast · Outdoor`，`nightlife → Bar/Party · Club · Music · Comedy · Cinema`，`film → Cinema · Music · Landmark · Shopping`。

只要 persona 信号非空，卡片网格上方会出现 `✦ Sorted for you` 横幅；点旁边的按钮即可切回默认插入顺序。
